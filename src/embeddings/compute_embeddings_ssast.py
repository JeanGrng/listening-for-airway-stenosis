"""Compute SSAST embeddings for B2AI Voice recordings and save to disk.

Loads SSAST-Base-Frame-400 weights (self-supervised) into the HuggingFace
ASTModel using the key-conversion approach, then extracts CLS embeddings.

Usage:
    python compute_embeddings_ssast.py --output embeddings_ssast.npz
    python compute_embeddings_ssast.py --output embeddings_ssast.npz --device cuda
"""
...
def load_ssast_model(weights_path, device):
    """Load SSAST weights into a HuggingFace ASTModel."""
    config = ASTConfig(
    ...
    # Monkey-patch for SSAST rectangular kernel (128, 2), HF ASTModel only supports square patches
    model.embeddings.patch_embeddings.projection = nn.Conv2d(
        1, 768, kernel_size=(128, 2), stride=(128, 2)
    )
    # 512 patches + 2 special tokens (CLS + distillation) = 514 position embeddings
    model.embeddings.position_embeddings = nn.Parameter(torch.zeros(1, 514, 768))
    ...
    print(f"Loaded SSAST weights from {weights_path}")
...
def linear_spectrogram_to_mel(spec_db, n_mels=128, sample_rate=16000, target_hop_ms=10, current_hop_ms=20):
    ...
    mel_fb = torchaudio.functional.melscale_fbanks(
        ...
        n_mels=n_mels_native, sample_rate=sample_rate,
    )
...
def main():
    parser = argparse.ArgumentParser(description="Canonical SSAST embeddings")
    parser.add_argument("--weights", type=str, default="weights/SSAST-Base-Frame-400.pth")
    parser.add_argument("--output", type=str, default="embeddings_ssast.npz")
    ...
    model = load_ssast_model(args.weights, device)
    print(f"SSAST on {device}", flush=True)
